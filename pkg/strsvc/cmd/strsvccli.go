...
func main() {
	...
	fs := flag.NewFlagSet("strsvc", flag.ExitOnError)
	var (
	...
	var (
		service        = strsvc.NewService(logger, ints, chars)
		endpoints      = strsvc.NewEndpoint(service, logger, duration, tracer, zipkinTracer)
		httpHandler    = strtransport.NewHTTPHandler(endpoints, tracer, zipkinTracer, logger)
		grpcServer     = strtransport.NewGRPCServer(endpoints, tracer, zipkinTracer, logger)
	)
	...
	}
	{
		...
		g.Add(func() error {
			...
			pb.RegisterStrServer(baseServer, grpcServer)
